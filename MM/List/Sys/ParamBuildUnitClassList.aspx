<%@ Page Title="建设单位类别参数设置" Language="C#" MasterPageFile="~/UI/Master/ObjectList.Master" AutoEventWireup="true" CodeBehind="ParamBuildUnitClassList.aspx.cs" Inherits="Zhongsoft.Portal.MM.List.Sys.ParamBuildUnitClassList" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="toolBar" runat="server">
    <span class="filter-block"><span class="filter-block-lb">建设单位</span><span>
        <zhongsoft:LightTextBox runat="server" ID="tbBuildUnit" CssClass="kpms-textboxsearch" EnableTheming="false"/>
    </span></span>
</asp:Content>
...
<asp:Content ID="Content5" ContentPlaceHolderID="listGrid" runat="server">
    <zhongsoft:LightPowerGridView runat="server" ID="gvList" UseDefaultDataSource="True" AllowPaging="True" PageSize="15" OnRowDataBound="gvList_OnRowDataBound" 
                                  BindGridViewMethod="BindData" DataKeyNames="ParamBuildUnitClassID" OnRowCommand="gvList_OnRowCommand">
        <Columns>
            ...
            <zhongsoft:LightButtonField HeaderText="查看" EditItemClick="showDetailItem" ToolTip="查看" DataParamFields="ParamBuildUnitClassID"
                                        ImageUrl="../../../Themes/Images/btn_dg_view.gif" ItemStyle-Width="30px" >
            </zhongsoft:LightButtonField>
            <zhongsoft:LightButtonField HeaderText="编辑" ToolTip="编辑" EditItemClick="editDetailItem" DataParamFields="ParamBuildUnitClassID"
            ...
            <zhongsoft:LightTemplateField HeaderText="删除" ItemStyle-Width="30px" ItemStyle-HorizontalAlign="Center">
                <ItemTemplate>
                    <asp:LinkButton ID="lbtnDelete" runat="server" CommandName="removeData" CommandArgument='<%#Eval("ParamBuildUnitClassID")%>'
            ...
            <zhongsoft:LightBoundField HeaderText="建设单位" DataField="BuildUnit" ItemStyle-Width="80px"/>
            ...
            <zhongsoft:LightTemplateField HeaderText="是否启用">
                <ItemTemplate>
                    <asp:RadioButtonList runat="server" ID="rbl" RepeatDirection="Horizontal" key='<%#Eval("ParamBuildUnitClassID") %>'
                                         OnSelectedIndexChanged="rbl_OnSelectedIndexChanged" AutoPostBack="true">
                        <asp:ListItem Value="1">启用</asp:ListItem>
                        <asp:ListItem Value="0">不启用</asp:ListItem>
                    </asp:RadioButtonList>
                    <input type="hidden" runat="server" id="hiKeyId" value='<%#Eval("ParamBuildUnitClassID") %>' />
                </ItemTemplate>
            </zhongsoft:LightTemplateField>
    ...
    <script type="text/javascript">
        ...
        function showDetailItem(id) {
            var url = buildBizUrl(1, id, "/MM/List/Sys/ParamBuildUnitClassEditor.aspx", null);
            showDivDialog(url, null, 600, 400, null);
            return false;
        }

        function editDetailItem(id) {
            var url = buildBizUrl(3, id, "/MM/List/Sys/ParamBuildUnitClassEditor.aspx", null);
        ...
        function addDetailItem() {
            ...
            var url = buildQueryUrl("/MM/List/Sys/ParamBuildUnitClassEditor.aspx", param);
