<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="BidFileTree.aspx.cs"
    Inherits="Zhongsoft.Portal.DCMS.List.BidFileTree" %>
...
<body>
                            ...
                            <table>
                                <tr>
                                    <td>
                                        ...
                                        </asp:DropDownList>项目名称/编号
                                        <input type="text" id="txtKeyWord" style="width: 90px" />
                                        <a onclick="searchTree()" id="btnSearch" class="searchbtn"><span>查找</span></a><a
                                            onclick="resetTree()" id="btnReset" class="searchbtn bluecolor"><span>重置</span></a>
    ...
    function getUrl(treeId, treeNode) {
        ...
        return "BidFileTree.aspx?keyword=" + encodeURI($hikeyword.val()) + "&secid=" + secid + "&type=" + typeValue + "&projectPhaseEntityID=" + '<%=OrganizationID %>';
    }
    ...
    $(document).ready(function () {
        ...
        $.post("BidFileTree.aspx?organizationID=" + '<%=OrganizationID %>' + "&rollID=" + '<%=RollID %>' + "&specialtyID=" + '<%=SpecialtyOfProjectEntityID %>' + "&externalFileID=" + "&mutualFileID=",
                       { asyfunc: "GetProject" },
                           function (res) {
                               ...
                               if ('<%=OrganizationID %>' != '') {
                                   ...
                                   //alert(treeObj.getNodes().length);
                                   node = treeObj.getNodes()[0];
                                   //node = treeObj.childNodes[0];
                                   treeObj.selectNode(node);
                                   treeObj.setting.callback.onClick = openUrl(node.url);
                               }
    ...
    function resetTree() {
        ...
        $.post("BidFileTree.aspx?organizationID=" + '<%=OrganizationID %>',
                    { asyfunc: "GetProject" },
                       function (res) {
                           $.fn.zTree.init($("#treeDemo"), setting, res);
                       }, "json")
    ...
    function searchTree() {
        ...
        if (word != "") {
            $hikeyword.val(word);
            $.fn.zTree.destroy("treeDemo");
            $.post("BidFileTree.aspx?secid=" + secid + "&type=" + typeValue + "&organizationID=" + '<%=OrganizationID %>',
