<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ProjectTreeList.aspx.cs" Inherits="Zhongsoft.Portal.EPMS.List.Project.ProjectTreeList" %>
...
<body>
    <form id="form1" runat="server">
        <div id="metroheader" style="height: 10px;">
        </div>
        <div>
                ...
                <table width="100%">
                    <tr>
                        <td width="20%" valign="top" style="border: 1px #6699FF solid;" id="tdTree">
                            <div id="prolefttree" style="padding: 0px; margin: 0px; width: 300px; overflow: auto; vertical-align: top; border: #a6c9e2 1px solid; background: #fff; height: 100%;">
                                <table>
        ...
        <script type="text/javascript">
            ...
            function getUrl(treeId, treeNode) {
                ...
                return "ProjectTreeList.aspx?keyword=" + encodeURI($hikeyword.val()) + "&secid=" + secid + "&type=" + typeValue;
            }
            ...
            $(document).ready(function () {
                ...
                $.post("ProjectTreeList.aspx?1=1",
                    { asyfunc: "GetProject" },
                    function (res) {
                        $.fn.zTree.init($("#treeDemo"), setting, res);
                        var treeObj = $.fn.zTree.getZTreeObj("treeDemo");
                        var node = treeObj.getNodeByParam('name', "全院生产项目", null);
                ...
                getArgs();
                //当前页面的查询内容比较窄，所以需要更新当前页面查询内容的最小宽度
                updateIframeStyle();
            });
            ...
            function searchTree() {
                ...
                if (word != "") {
                    $hikeyword.val(word);
                    $.fn.zTree.destroy("treeDemo");
                    $.post("ProjectTreeList.aspx?secid=" + secid + "&type=" + typeValu,
                        { keyword: word, asyfunc: "GetSearch" },
                        function (res) {
                            $.fn.zTree.init($("#treeDemo"), setting, res);
            ...
            //当前页面的查询内容比较窄，所以需要更新当前页面的最小宽度。标准宽度：800px。
            function updateIframeStyle() {
                var iframe = document.getElementById("workspace");
                if (iframe.attachEvent) {
                    iframe.attachEvent("onload", function () {
                        $("#divBars", document.frames('workspace').document).css("min-width", "680px");
                    });
                } else {
