<%@ Page Title="已办流程" Language="C#" MasterPageFile="~/UI/Master/ObjectList.Master"
    AutoEventWireup="true" CodeBehind="WorkProcessDoneList.aspx.cs" Inherits="Zhongsoft.Portal.Sys.Workflow.WorkProcessDoneList" %>
   ...
   <div style="background-color: #EFF2F7;">
        <span class="filter-block"><span class="filter-block-lb">发起人</span> <span>
            ...
            发起部门</span> <span>
                ...
                关&nbsp;注</span><span>
                     <asp:DropDownList runat="server" ID="ddlAttention" Width="60px" class="kpms-ddlsearch" EnableTheming="false">
                    <asp:ListItem Value="">全部</asp:ListItem>
                ...
                接收时间</span><span>
                     <asp:DropDownList runat="server" ID="ddlReceiveDate" Width="60px"  class="kpms-ddlsearch" EnableTheming="false">
...
<asp:Content ID="Content2" ContentPlaceHolderID="listGrid" runat="server">
    ...
    <zhongsoft:LightPowerGridView ID="PrcDoneList" runat="server" AllowPaging="true"
        ...
        <Columns>
              <asp:TemplateField HeaderStyle-Width="20px" Visible="false">
                <HeaderTemplate>
                   查看表单
                </HeaderTemplate>
                <ItemTemplate>
                    <%# GetNewToDoItem(Eval("FormId").ToString(),
                      "", 
                     Eval("ProcessInstanceId").ToString(), 
                     "<span id=‘span2' style='color: blue;'> 查看表单 </span>", 
                      string.Empty)%>
                </ItemTemplate>
            </asp:TemplateField>
            ...
            <asp:TemplateField>
                <HeaderTemplate>
                    <%--<img src="../../Themes/Images/btn_notattention.png" title="关注情况" alt="关注情况">--%>关注
                </HeaderTemplate>
                <ItemTemplate>
           ...
           <asp:TemplateField HeaderText="事项主题" >
                <ItemTemplate>
                      ...
                      "", 
                     Eval("ProcessInstanceId").ToString(), 
                     "<span id=‘span2' style='color: blue;'>" +  Server.HtmlEncode(Convert.ToString(Eval("ProcessInstanceName"))) +" </span>", 
                      string.Empty)%> 
                </ItemTemplate>
            </asp:TemplateField>
            <asp:TemplateField HeaderText="事项主题"  Visible="false">
                <ItemTemplate>
                    <div style="width:500px;" class="autoskip"><a href="javascript:void(0)" isshow="0" prc="<%#Eval("ProcessInstanceId") %>" title='<%# Server.HtmlEncode(Convert.ToString(Eval("ProcessInstanceName")))%>'>
            ...
            <asp:TemplateField HeaderText="发起人[所在部门/科室]发起时间" HeaderStyle-Width="350px">
                <ItemTemplate>
                    <%#Eval("CreatorName")%>&nbsp;[<%#GetDeptName(Eval("CreatorId").ToString(),Eval("CreatorOrganizationName").ToString()) %>]&nbsp;<%# GetStartDate(DateTime.Parse(Eval("StartDateTime").ToString()))%>
                </ItemTemplate>
            </asp:TemplateField>
